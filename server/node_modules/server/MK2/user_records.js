...
router.get("/mk2_users", async (req, res) => {
  try {
...
router.post("/users/mk2_add", async (req, res) => {
  try {
...
router.delete("/users/mk2_delete/:id", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await pool.request()
      .input("id", sql.Int, id)
      .query(`DELETE FROM ANDON_USER_LOGIN WHERE ID = @id`);
