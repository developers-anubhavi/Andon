...
const { poolPromise } = require("../MK2/db");

router.get("/mk2", async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT
        ...
        MAIN3_ACTUAL,
        MAIN2_STOPTIME_HOUR,
        MAIN2_STOPTIME_MINUTES,
        MAIN3_STOPTIME_HOUR,
        MAIN3_STOPTIME_MINUTES,
        OVER_TIME
      FROM COMMON_TABLE
    `);
    ...
    res.status(200).json({
      mk2_today_data: row.TODAY,
      mk2_present_data: row.MAIN2_PRESENT,
      mk2_main2_actual_data: row.MAIN2_ACTUAL,
      mk2_main3_actual_data: row.MAIN3_ACTUAL,
      mk2_main2_oa_data: row.MAIN2_OA,
      mk2_main2_np_data: row.MAIN2_NP,
      mk2_main2_pf_data: row.MAIN2_PF,
      mk2_main2_stop_time_data: row.MAIN2_STOPTIME_HOUR,
      mk2_main2_stop_time_minutes_data: row.MAIN2_STOPTIME_MINUTES,
      mk2_main3_oa_data: row.MAIN3_OA,
      mk2_main3_np_data: row.MAIN3_NP,
      mk2_main3_pf_data: row.MAIN3_PF,
      mk2_main3_stop_time_data: row.MAIN3_STOPTIME_HOUR,
      mk2_main3_stop_time_minutes_data: row.MAIN3_STOPTIME_MINUTES,
      mk2_over_time_data: row.OVER_TIME,
    });
  } catch (error) {
    console.error(error);
...
router.get("/mk2_boxes", async (req, res) => {
  try {
    const pool = await poolPromise;
    ...
    const result = await pool.request().query(`
      ...
      FROM MK2MK3_COLOR_CALL_MONITORING
      ORDER BY DATE_TIME DESC  -- latest entry
    `);
 ...
 router.get("/mk2_boxes_colors", async (req, res) => {
  try {
    const pool = await poolPromise;
...
// router.get("/mk2_boxes_cumduration", async (req, res) => {
//   try {
//     const pool = await poolPromise;
//     const result = await pool.request().query(`
//       SELECT TOP 1
...
// });
 ...
 router.get("/mk2_red_count", async (req, res) => {
  try {
    const pool = await poolPromise;
...
router.get("/mk2_yellow_count", async (req, res) => {
  try {
    const pool = await poolPromise;
 ...
 router.get("/mk2_red_time", async (req, res) => {
  try {
    const pool = await poolPromise;
...
router.get("/mk2_yellow_time", async (req, res) => {
  try {
    const pool = await poolPromise;
