...
const { poolPromise } = require("../ET/db");
...
router.get("/et", async (req, res) => {
  try {
    ...
    res.status(200).json({
      et_today_data: row.TODAY,
      et_present_data: row.ET_PRESENT,
      et_actual_data: row.ET_ACTUAL,
      et_over_time_data: row.OVER_TIME,
      et_stop_time_data: row.MAIN3_STOPTIME_HOUR,
      et_stop_time_minutes_data: row.MAIN3_STOPTIME_MINUTES,
      et_main3_actual_data: row.MAIN3_ACTUAL,
      et_oa_data: row.ET_OA
    });
  } catch (error) {
    console.error(error);
...
router.get("/et_boxes", async (req, res) => {
  try {
    const pool = await poolPromise;
    ...
    const result = await pool.request().query(`
      ...
      FROM SPSET_COLOR_CALL_MONITORING
...
router.get("/et_boxes_colors", async (req, res) => {
  try {
    const pool = await poolPromise;
...
// router.get("/et_boxes_cumduration", async (req, res) => {
//   try {
//     const pool = await poolPromise;
//     const result = await pool.request().query(`
//       SELECT TOP 1
...
// router.get("/et_red_count", async (req, res) => {
//   try {
//     const pool = await poolPromise;
...
router.get("/et_yellow_count", async (req, res) => {
  try {
    const pool = await poolPromise;
...
// Fetch latest 60 box names + colors
// router.get("/et_red_time", async (req, res) => {
//   try {
//     const pool = await poolPromise;
...
router.get("/et_yellow_time", async (req, res) => {
  try {
    const pool = await poolPromise;
