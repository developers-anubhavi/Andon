...
router.get("/mk1", async (req, res) => {
  try {
    ...
    res.status(200).json({
      mk1_today_data: row.TODAY,
      mk1_present_data: row.MAIN1_PRESENT,
      mk1_actual_data: row.MAIN1_ACTUAL,
      mk1_stop_time_data: row.MAIN1_STOPTIME_HOUR,
      mk1_stop_time_minutes_data: row.MAIN1_STOPTIME_MINUTES,
      mk1_OA_data: row.MAIN1_OA,
      mk1_over_time_data: row.OVER_TIME,
      mk1_pf_data: row.MAIN1_PF,
      mk1_np_data: row.MAIN1_NP
    });
  } catch (error) {
    console.error(error);
...
router.get("/mk1_boxes", async (req, res) => {
  try {
    const pool = await poolPromise;
    ...
    const result = await pool.request().query(`
      ...
      FROM MK1_COLOR_CALL_MONITORING
 ...
 router.get("/mk1_boxes_colors", async (req, res) => {
  try {
    const pool = await poolPromise;
...
// router.get("/mk1_boxes_cumduration", async (req, res) => {
//   try {
//     const pool = await poolPromise;
//     const result = await pool.request().query(`
//       SELECT TOP 1
...
// });
 ...
 router.get("/mk1_red_count", async (req, res) => {
  try {
    const pool = await poolPromise;
...
router.get("/mk1_yellow_count", async (req, res) => {
  try {
    const pool = await poolPromise;
 ...
 router.get("/mk1_red_time", async (req, res) => {
  try {
    const pool = await poolPromise;
...
router.get("/mk1_yellow_time", async (req, res) => {
  try {
    const pool = await poolPromise;
